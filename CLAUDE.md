...
**开工前第一件事：读 `develop_history/X_自定义游戏内容Mod开发/CLAUDE.md`**，
再按它指的顺序读 `.claude/PROGRESS.md` → `FINDINGS.md` → `DECISIONS.md` → `PLAN.md`。
当前分支是 **`develop/X_Mod` · 自定义游戏内容 Mod**；
V0.1 / V0.2 / V0.3（bot、合成与商店）已完工，在 `develop_history/` 里只读参考
（那几套代码仍在跑，那些 FINDINGS 仍然有效，引用时写全「V0.3商店 §50」）。
...
四条最容易致命的铁律（完整版在当前版本的 CLAUDE.md 里）：

1. `game_org/` 和 `原版安装包/` **只读**，改什么都去 `game_patched/`。
2. **不要让 GameGuard 跑起来** —— `game_patched/GameGuard.des` 必须保持改名状态。
3. `.bat` 用 **CRLF + UTF-8 无 BOM 且一个中文都不许有**（中文放同名 `.ps1`），
   `.ps1` 用 **CRLF + UTF-8 有 BOM**，`.py`/`.sh`/`.json`/`.md` 用 **LF 无 BOM**。
   ★ 写含中文的文件一律用 Write/Edit 工具 —— 这台机器的 Git Bash heredoc 按 CP936 落盘。
4. **不替用户动 git 履历**（commit / push / reset / rebase / …），改完停在工作区。
