...
耐久度 / 修理、附魔（`Enchant.ini` 客户端里不存在）、
期限制装备（`expires` 字段预留，初期一律永久）、
付费角色的期限制形态（`1019000xx` 那些只有货架条目；角色卡本身 D51 起在商店卖）。

★ 这几条后来**做掉了**，留着划掉的记号是为了让引用旧结论的人知道要回头改：

* ~~礼物系统~~ → D76 做了（管理页「批量发送奖励」→ 原版礼物盒）；
* ~~宠物~~ → 合成产线里有 7 个；
* ~~称号 + 结算界面的「称号卡片」槽~~ → **2026-09-13 做了**（D98 ~ D105）：
  17 条称号配方 + `cards.json` 第七份运营配置 + `0x041c` 的 `slot=1` 通电。
  当初砍掉它们的理由是「没有成就统计」（D44a），这一版正是去补那个前提。
